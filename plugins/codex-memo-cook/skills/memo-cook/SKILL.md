...
Memo Cook is the user's shared memory outboard for Codex and other agents. It stores human-readable Markdown notes, copies image attachments, fetches URL text/metadata, and maintains a rebuildable SQLite FTS index.

## Operating Rules

- Use `memo_search`, `memo_read`, and `memo_list_inbox` freely when the user asks what is remembered or when durable context would help the current task.
